...
1. 변경사항 파악
2. changeset 추가 (`bun changeset`) — `src/` 변경이 있는 경우
3. 커밋 & push
4. develop 대상으로 PR 생성

### changeset 판단 기준

`src/` 내부 변경 여부로 자동 판단한다 (`check-changeset.sh`). 개발자는 설명이 사용자 관점으로 작성됐는지만 확인한다.
...
| `src/` 변경 없음 | 생략 | `.claude/`, CI, 문서 등만 수정한 경우 |

> pre-push 훅이 검증한다. `src/` 변경이 있는데 changeset 파일이 없으면 push가 차단된다.
